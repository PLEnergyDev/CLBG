...
static void repeat_fasta(const char *seq,
    ...
    for (i=0; i< whole_buffers; i++) *res +=(buflen2);

    int data_remaining = n - whole_buffers * len * LINELEN;
    int embedded_newlines = data_remaining / LINELEN;
    *res += data_remaining + embedded_newlines;

    free(buffer1);
    free(buffer2);
    if (n % LINELEN != 0) *res += 1;
}
...
int Fasta(ulong loopIterations) {
    ...
    for(ulong i = 0; i < loopIterations; i++){
        result += (sizeof(header1)-1);
        repeat_fasta(alu, n*2, res_ptr);

        result += (sizeof(header2)-1);
        random_fasta(iub, iub_p, n*3, res_ptr);

        result += (sizeof(header3)-1);
        random_fasta(homosapiens, homosapiens_p, n*5, res_ptr);
    }
